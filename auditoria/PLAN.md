...
- [ ] **Migrar `middleware.ts` a la nueva convención de Next.js 16:** Investigar y aplicar el sistema de `proxy` para evitar avisos de deprecación.
- [x] **Refactorizar Circuit Breaker (HALF_OPEN):** 
  ...
  - [ ] Implementar un log específico para transiciones de estado del circuito.
- [ ] **Validación de Entorno en Build:** Crear un script `src/shared/lib/env.ts` que valide la presencia de variables críticas al arrancar la app.
...
- [x] **Estandarizar Componentes de Tabla:** 
  ...
  - [ ] Añadir soporte para "Sticky Headers" nativo en el CSS Module.
- [ ] **Unificar Formularios:** 
  - [ ] Migrar todos los formularios de features (`DigitalWalletForm`, `BankAccountForm`) para usar el componente `Form.tsx` modular.
- [ ] **Tipado Estricto de Dictionaries:** Crear un tipo global derivado de `es.json` para evitar el uso de `any` en los diccionarios de i18n.
